<script lang="ts">
  ...
  import type { IpDataType } from "./types/ipData";

  // constants
  const API_KEY_IPIFY = globalThis.env.API_KEY_IPIFY;
  const API_KEY_MAPBOX = globalThis.env.API_KEY_MAPBOX;
  const fadeUpIn = { y: 30, duration: 500 };

  // state
  let map: Map;
  let locationMarker: Marker;
  let ipData: IpDataType = {
    ip: "",
    isp: "",
    name: "",
    location: {
  ...
  async function getIP(): Promise<void> {
    ...
    try {
      const response = await axios.get(
        `https://geo.ipify.org/api/v1?apiKey=${JSON.parse(
          API_KEY_IPIFY
        )}${domain}`
      );
      const { data } = response;
  ...
  function initMap() {
    ...
    const { lng, lat } = ipData.location;
    mapboxgl.accessToken = JSON.parse(API_KEY_MAPBOX);
...
<main>
  <header>
    ...
    <div class="search-wrapper">
      <input
        type="text"
        placeholder="Search for IP address by domain"
